...
export default function currenAccountClient1Pay({ sales, salesTarjets, salesCheqs }) {
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 10;

  const filteredSales = sales.filter((sale) => sale.name === 'Gabriel' || sale.lastname === 'Alvarez' || sale.product === 'CC');
  const filtereSalesTarjets = salesTarjets.filter((salesTarjet) => salesTarjet.name === 'Gabriel' || salesTarjet.lastname === 'Alvarez' || salesTarjet.product === 'CC');
  const filtereSalesCheqs = salesCheqs.filter((salesCheq) => salesCheq.name === 'Gabriel' || salesCheq.lastname === 'Alvarez' || salesCheq.product === 'CC');

  const allSales = [...filteredSales, ...filtereSalesTarjets, ...filtereSalesCheqs];
      ...
      <div>
        <div>
          <button className="btn btn-secondary float-start">
            <Link href="/currenAccountClient1" style={{ textDecoration: 'none', color: 'white' }}>
              <ArrowBackIcon />
            </Link>
          </button>
        </div>
        <div style={{ width: "80%", margin: "auto" }}>
          ...
          <div style={{ textAlign: "center" }}>
            {currentPage > 1 && (
              <button className="btn btn-primary mr-2" onClick={() => paginate(currentPage - 1)}>Anterior</button>
            )}
            {currentRecords.length === recordsPerPage && (
              <button className="btn btn-primary" onClick={() => paginate(currentPage + 1)}>Siguiente</button>
            )}
          </div>
        </div>
      </div>
